...
run_solution <- function(x.set, d.t=1e-3, Time=0.4049248, theta=1,
                         S=function(t){return(2)},
                         trajectory_name="constant", 
	...
	if(length(x.set)==length(eq_dist_emp)){
        ...
        G.0 <- eq_dist_fun(x=x.set,theta=theta,S=S(0))*x.set*(1-x.set);
    }
    ...
    A <- A_diag_matrix(x=x.set,d.t=d.t,S=S(d.t),t.curr=d.t,edges=edges,
                       use.upwind=upwind.by.sel,rho=rho);
    
    ## INITIALIZE THE d VECTOR
    d <- d_basic_rev(G.current=G.0, G.0bound=theta*rho(d.t), G.Nupbound=0, 
                     x=x.set, d.t=d.t, d.x.0=d.x.0, d.x.N=d.x.final, S=S(d.t), t.curr=d.t, 
                     use.upwind=upwind.by.sel, rho=rho);
    ...
    for(i in 2:length(t.set)){
        print(i)
        A <- A_diag_matrix(x=x.set,d.t=d.t,S=S(t.set[i]),t.curr=t.set[i],edges=edges,
                           use.upwind=upwind.by.sel,rho=rho);
        d <- d_basic_rev(G[,i-1], G.0bound=theta*rho(t.set[i]), G.Nupbound=0, 
                         x=x.set, d.t=d.t, d.x.0=d.x.0, d.x.N=d.x.final, S=S(t.set[i]),
                         t.curr=t.set[i],
                         use.upwind=upwind.by.sel, rho=rho);
        G[,i] <- solve(a=A,b=d);
	## ~~~ CHECKS FOR SOLUTION STABILITY ~~~ ##
    ...
    if(eq_dist_emp_fname!=0){
        ...
        fname.out <- paste("../generated_data/T",theta,"S",
                           S(0),"traj-",trajectory_name,"emp-",
                           simple.name,".dat",sep="");
        foo <- write.table(G,file=fname.out,sep=" ",row.names=FALSE,col.names=FALSE);
    } else{
        fname.out <- paste("../generated_data/T",theta,"S",S(0),"traj-",
